...
import { pathToCLIAssets, pathToCluster } from './paths.js';
import { createKeyPair, createNodeName } from './util.js';
...
export default class Cluster {
   ...
   connectionInfo(): {
      ...
      return {
         ...
         privateKey: readFileSync(path.join(this.path, 'cluster_key'), 'utf8'),
         username: 'dev'
      };
   }
   ...
   create(options: ClusterOptions): boolean {
      try {
         ...
         mkdirSync(path.join(this.path, 'hostkeys'), { recursive: true });
         mkdirSync(path.join(this.path, 'conf'), { recursive: true });

         //  Generate cluster keys
         createKeyPair(authorizedKeys, path.join(this.path, 'cluster_key'));

         // Generate login node keys
         const loginKeyPair = createKeyPair(
            authorizedKeys,
            path.join(this.path, 'hostkeys', 'login_ssh_host_ed25519_key')
         );
         appendFileSync(path.join(this.path, 'known_hosts'), `login ${loginKeyPair.public}\n`);

         // Generate database keys
         createKeyPair(
            authorizedKeys,
            path.join(this.path, 'hostkeys', 'database_ssh_host_ed25519_key')
         );
         ...
         for (let i = 1; i <= options.nodes; i++) {
            ...
            const keyPair = createKeyPair(
               authorizedKeys,
               path.join(this.path, 'hostkeys', `${nodeName}_ssh_host_ed25519_key`)
            );

            appendFileSync(path.join(this.path, 'known_hosts'), `${nodeName} ${keyPair.public}\n`);
         }

         // Generate key for moving between clusters
         createKeyPair(authorizedKeys, path.join(this.path, 'shared_cluster_key'));
         ...
         // Create compose.yaml
         writeFileSync(path.join(this.path, 'compose.yaml'), eta.render('creation', options));
         ...
         cpSync(
            pathToCLIAssets(__dirname, 'setup_scripts'),
            path.join(this.path, 'setup_scripts'),
            {
               recursive: true
            }
         );
         ...
         writeFileSync(
            path.join(this.path, 'conf', 'slurm.conf'),
            eta.render('slurmconf', options)
         );
         if (options.database) {
            writeFileSync(
               path.join(this.path, 'conf', 'slurmdbd.conf'),
               eta.render('slurmdbd', options)
            );
         }

         // Create cluster information file
         writeFileSync(path.join(this.path, 'info.json'), JSON.stringify(options));

         // Generate munge key
         writeFileSync(path.join(this.path, 'munge.key'), randomBytes(256));
