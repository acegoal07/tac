...
export default class CreateIndex extends Command {
   ...
   static override readonly flags = {
      count: Flags.integer({ char: 'c', default: 1, description: "How many nodes to give to the cluster", min: 1 }),
      memory: Flags.integer({ char: "m", default: 1024, description: "How much memory will be given to the cluster", min: 1024 }),
      module: Flags.string({ char: "N", default: 'lmod', description: "The module loader type to use in the cluster" }),
      name: Flags.string({ char: 'N', default: "KindOfaCluster", description: "The name of the cluster", required: true })
   }
   ...
   public async run(): Promise<void> {
      ...
      writeFileSync(join(clusterPath, 'hostkeys', 'login_ssh_host_ed25519_key.pub'), loginKey.public);
      ...
      for (let i = 1; i <= flags.count; i++) {
         ...
         writeFileSync(join(clusterPath, 'hostkeys', `node${String(i).padStart(2, "0")}_ssh_host_ed25519_key`), keys.private);
         chmodSync(join(clusterPath, 'hostkeys', `node${String(i).padStart(2, "0")}_ssh_host_ed25519_key`), 0o600);
         writeFileSync(join(clusterPath, 'hostkeys', `node${String(i).padStart(2, "0")}_ssh_host_ed25519_key.pub`), keys.public);
      ...
      writeFileSync(join(clusterPath, 'compose.yaml'),
         eta.render('creation.ts.eta', flags)
      );

      spinner.succeed('Generated compose file');
      spinner = ora('Copying docker files').start();

      copyFileSync(join(__dirname, '..', '..', 'assets', 'docker', 'Dockerfile'), join(clusterPath, 'Dockerfile'));
      copyFileSync(join(__dirname, '..', '..', 'assets', 'docker', 'entrypoint.sh'), join(clusterPath, 'entrypoint.sh'));
      cpSync(join(__dirname, '..', '..', 'assets', 'setup_scripts'), join(clusterPath, 'setup_scripts'), { recursive: true });
      ...
      writeFileSync(join(clusterPath, 'conf', 'slurm.conf'),
         eta.render('slurmconf', flags)
      )

      writeFileSync(join(clusterPath, 'conf', 'slurmdbd.conf'),
         eta.render('slurmdbd', flags)
      )
